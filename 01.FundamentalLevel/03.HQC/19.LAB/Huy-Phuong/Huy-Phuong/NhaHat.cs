...
namespace Huy_Phuong
{
                    ...
                    try
                    {
                        switch (chiHuy)
                        {
                            case "AddTheatre": chiHuyParts = chiHuyLine.Split('('); 
                                ...
                                string[] chiHuyParts1 = chiHuyLine.Split(new[] { '(', ',', ')' }, StringSplitOptions.RemoveEmptyEntries); string[] chiHuyParams1 = chiHuyParts1.Skip(1).Select(p => p.Trim()).ToArray();
    ...
    public class BuoiDien : IComparable<BuoiDien>
    {
        ...
        {
            string result = string.Format("BuoiDien(Tr32: {0}; Tr23: {1}; s2: {2}, ThoiGian: {3}, Gia: {4})",
this.tr23, this.tr32, this.s2.ToString("dd.MM.yyyy HH:mm"), this.ThoiGian.ToString("hh':'mm"), this.gia.ToString("f2"));
            return result;
        }
    }
